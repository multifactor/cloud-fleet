...
export function viewerScriptText({ tmuxBin = 'tmux', socket, configFile, session, windowId, label }) {
  const view = viewerSessionName(label)
  const argv = [
    tmuxBin, '-L', socket, '-f', configFile,
    'new-session', '-A', '-s', view, '-t', `=${session}`,
  ].map(shQuote).join(' ')
  return [
    '#!/bin/sh',
    ...
    'unset TMUX',
    '',
    '# ⛔ A VIEW MUST DIE WITH THE THING IT VIEWS. A grouped session shares the fleet\'s whole window',
    '# list, so killing the session\'s window does NOT end the view: tmux quietly moves this client to',
    '# some other window, and the operator is left with stale terminals all showing the same surviving',
    '# session. After a reclaim that is worse than useless — the windows say the fleet is still busy',
    '# with work that finished. So watch for the window going away and end the view with it.',
    `while ${shQuote(tmuxBin)} -L ${shQuote(socket)} list-windows -a -F '#{window_id}' 2>/dev/null | grep -qx ${shQuote(windowId)}; do sleep 3; done &&`,
    `  ${shQuote(tmuxBin)} -L ${shQuote(socket)} kill-session -t ${shQuote('=' + view)} >/dev/null 2>&1 &`,
    '',
    // ⛔ `exec`, so tmux BECOMES the shell. Left as a child of an interactive shell, Terminal keeps
    // the window when tmux exits — "[exited]" and a prompt, a dead terminal that reads as a live one
    // — and closing a window that still holds a process raises "terminate running processes?", one
    // modal per window. With tmux as the shell there is nothing left to terminate, and
    // ensureTerminalClosesOnExit() makes Terminal close the window rather than keep it.
...
/**
 * Make Terminal close a viewer window when its tmux client exits.
 *
 * ⛔ TWO DIALOGS, ONE CAUSE, BOTH LANDING ON THE OPERATOR. Terminal's default is to KEEP a window
 * whose shell has exited: it prints "[exited]" and drops to a prompt, so a dead viewer looks exactly
 * like a live one until you read it — and after a reclaim the screen fills with them. Closing such a
 * window while anything still runs in it raises "Do you want to terminate running processes?", a
 * modal PER WINDOW on a machine that may have one per session. `shellExitAction: 0` answers the
 * first; the script's `exec` answers the second.
 *
 * Idempotent, one key, only the profile Terminal actually opens with, and never fatal — a stale
 * window is a nuisance, not a broken fleet.
 */
export function ensureTerminalClosesOnExit({ exec = run, env = process.env, platform = process.platform } = {}) {
  if (platform !== 'darwin') return { changed: false, reason: null }
  const home = env.HOME
  if (!home) return { changed: false, reason: 'no HOME' }
  const plist = `${home}/Library/Preferences/com.apple.Terminal.plist`
  const read = exec('defaults', ['read', 'com.apple.Terminal', 'Default Window Settings'], { timeoutMs: 10_000 })
  if (!read.ok) return { changed: false, reason: 'Terminal has no saved default profile yet' }
  const profile = String(read.stdout || '').trim()
  if (!profile) return { changed: false, reason: 'no default profile name' }
  const key = `:'Window Settings':'${profile}':shellExitAction`
  const cur = exec('/usr/libexec/PlistBuddy', ['-c', `Print ${key}`, plist], { timeoutMs: 10_000 })
  if (cur.ok && String(cur.stdout).trim() === '0') return { changed: false, profile, reason: 'already closes on exit' }
  const verb = cur.ok ? `Set ${key} 0` : `Add ${key} integer 0`
  const w = exec('/usr/libexec/PlistBuddy', ['-c', verb, plist], { timeoutMs: 10_000 })
  return w.ok ? { changed: true, profile, reason: null } : { changed: false, profile, reason: (w.stderr || '').trim() || `exit ${w.code}` }
}
...
export function createTmuxBackend({
  ...
  function openViewer(windowId, label) {
    ...
    try {
      // One key, once, before the first window opens: otherwise every viewer leaves a dead
      // "[exited]" window behind and every close raises a terminate-processes modal.
      if (kind === 'terminal-app') ensureTerminalClosesOnExit({ exec, env, platform })
